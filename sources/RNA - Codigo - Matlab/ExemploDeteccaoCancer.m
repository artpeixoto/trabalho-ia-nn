...
% download and uncompress the raw mass-spectrometry data from the FDA-NCI
% web site. Create the data file *OvarianCancerQAQCdataset.mat* by either
% running script *msseqprocessing* in Bioinformatics Toolbox (TM) or by
% following the steps in the example *biodistcompdemo* (Batch processing
% with parallel computing). The new file contains variables |Y|, |MZ| and
% |grp|.
%
...
%
%   ind = rankfeatures(Y,grp,'CRITERION','ttest','NUMBER',100); x =
%   Y(ind,:);
%
% Define the targets |t| for the two classes as follows:
%
%   t = double(strcmp('Cancer',grp)); t = [t; 1-t];
%
...
% and feature selection procedures. Using different steps or parameters may
...
%
% Performance is measured in terms of mean squared error, and shown in a log
...
%
% The network outputs will be in the range 0 to 1, so we threshold them to
% get 1's and 0's indicating cancer or normal patients respectively.
